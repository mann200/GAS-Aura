...
void AAuraProjectile::Destroyed()
{
	...
	// 为什么重写Destroyed函数，因为客户端有两种情况，一种是在服务器调用销毁函数时，还没有触发碰撞，另一种是先于服务器触发了碰撞
	// 为了防止客户端在第一种情况下没有生成效果，所有重写销毁函数做保障
	// bHit变量是为了限制客户端只生成一次效果
...
{
	/*  // 这是原版教程的代码，客户端运行会有问题。DamageEffectSpecHandle是空指针
	...
	if (!bHit && HasAuthority())
	{
		//UE_LOG(LogTemp,Warning,TEXT("%ls"),*OtherActor->GetName());
		UGameplayStatics::PlaySoundAtLocation(this,ImpactSound,GetActorLocation(),FRotator::ZeroRotator);
		UNiagaraFunctionLibrary::SpawnSystemAtLocation(this,ImpactEffect,GetActorLocation());
	...
	*/
	...
	if (HasAuthority())
	{
		...
		// 此Actor是网络复制的，所以服务器调用销毁，客户端也会销毁
		Destroy();
	}
	else
	{
