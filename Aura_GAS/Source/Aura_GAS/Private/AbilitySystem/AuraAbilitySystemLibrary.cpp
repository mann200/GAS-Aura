...
void UAuraAbilitySystemLibrary::InitializeDefaultAttrabutes(const UObject* WorldContextObject,ECharacterClass CharacterClass, float Level,UAbilitySystemComponent* ASC)
{
...
void UAuraAbilitySystemLibrary::InitializeDefaultAttrabutesFromSaveData(const UObject* WorldContextObject,
...
void UAuraAbilitySystemLibrary::SetDeathImpulseDirection(FDamageEffectParams& DamageEffectParams,FVector DeathImpulseDirection,float Magnitude)
{
	...
	if(Magnitude == 0.f)
	{
		DamageEffectParams.DeathImpulse = DamageEffectParams.DeathImpulseMagnitude * DeathImpulseDirection;
	}
	{
...
{
	const AAura_GASGameModeBase* AuraGameMode = Cast<AAura_GASGameModeBase>(UGameplayStatics::GetGameMode(WorldContextObject));
	...
	FVector InRadius)
{
	if(FAuraGameplayEffectContext* AuraEffectContext = static_cast<FAuraGameplayEffectContext*>(EffectContextHandle.Get()))
	{
		AuraEffectContext->SetRadialDamageOrigin(InRadius);
	}
}
...
void UAuraAbilitySystemLibrary::GetColsestTarget(int32 MaxTargets, const TArray<AActor*>& Actors,
