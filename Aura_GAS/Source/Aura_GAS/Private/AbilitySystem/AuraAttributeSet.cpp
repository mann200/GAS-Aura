...
void UAuraAttributeSet::Debuff(const FEffectProperties& Props)
{
	...
	//InheritableOwnedTagsContainer函数5.3版本已经弃用 需要寻找其他方法替代
	...
	if (FGameplayEffectSpec* MutableSpec = new FGameplayEffectSpec(DebuffEffect,EffectContextHandle,1.0f))
	{
		FAuraGameplayEffectContext* AuraContext = static_cast<FAuraGameplayEffectContext*>(MutableSpec->GetContext().Get());
		TSharedPtr<FGameplayTag> DebuffDamageType = MakeShared<FGameplayTag>(DamageType);
		AuraContext->SetDamageType(DebuffDamageType);
		// DynamicGrantedTags可替代上面的InheritableOwnedTagsContainer
